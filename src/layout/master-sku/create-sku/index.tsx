...
import { Button, Input, InputSelect, ModalConfirmPopup, ModalConfirm } from '@/components';
...
const CreateSku = ({
  ...
  const handleValidation = () => {
    ...
    const editData = `${JSON.stringify(dataForm)}${JSON.stringify(
      listProductSync
    )}${JSON.stringify(setToJsonValidation)}`;
      ...
      if (formEdit === editData) {
        ...
        const isManagedByEthixInvalid = (dataForm) => {
          if (dataForm.managedByEthix === 'yes') {
            ...
            if (dataForm.managedByProduct === 'fifo' || dataForm.managedByProduct === '') {
  ...
  useEffect(() => {
    handleValidation(); 
  }, [dataForm, listProductSync, dataImagesUpload]);
