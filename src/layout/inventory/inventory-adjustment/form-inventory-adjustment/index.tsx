...
import { schemaInventoryAdjustment } from '@/utils/validation/yup-validation';
import { getOptionStockInventoryAdjustment } from '@/utils/getSelectOption';

// redux & service
import { useSelector } from 'react-redux';
import Product from '@/components/organism/product-sku-table/styles';
import { clearEmojiInput } from '@/utils/formater';
import { getAdjustmentDetail, postCreateAdjustment, patchAdjustment } from '@/services/inventory';
import { uniq, isEqual } from 'lodash';
import ListTableProduct from '@/components/molecules/table/table-inventory-adjustment/table-sku';
...
const InventoryAdjustmentForm = ({ 
  ...
  const [oldListSku, setOldListSku] = useState([]);
  ...
  const {
    ...
    mode: 'onBlur',
    resolver: yupResolver(schemaInventoryAdjustment),
  });
  ...
  const onFormSubmit = async (data: any) => {
    const payload = {
      client_id: client_id,
      location_id: parseInt(data.origin_warehouse),
      stock_source: data.stock_source,
      adjustment_date: moment(data.adjustment_date).format('DD/MM/YYYY') || '',
      external_id: data.external_id || '',
      notes: data.note || '',
      products: listSku.length
        ? listSku
            .filter((v) => v.quantity !== null && v.quantity !== undefined && v.quantity !== '')
            .map((v) => ({
              product_id: v.product_id,
              stock_adjustment: v.quantity,
            }))
        : [],
    };
    ...
    try {
      const response = edit
        ? await patchAdjustment(idSearch, payload)
        : await postCreateAdjustment(payload);
  ...
  const onChangeDeleteProduct = (id) => {
    !alreadyChangedSku && setalreadyChangedSku(true);
    let newList = [...listSku];
    newList.splice(id, 1);
    setListSku(newList);
  };

  const onAdjustmentStock = (id, type, value) => {
    !alreadyChangedSku && setalreadyChangedSku(true);
    let newList = [...listSku];
  ...
  const handleGetPurchaseDetail = async () => {
    await getAdjustmentDetail(idSearch).then((res) => {
      if (res?.status == 200) {
        const data = res.data;
        setWarehouseValue(data?.location_id);
        setValue('origin_warehouse', data?.location);
        setValue('adjustment_date', data?.adjustment_date);
        setValue('external_id', data?.external_id);
        setValue('stock_source', data?.stock_source);
        setListSku(
          data?.adjustment_details.map((v) => {
            return {
              ...v,
              quantity: `${v?.quantity}`,
            };
        ...
        setOldListSku(data?.adjustment_details);
  ...
  useEffect(() => {
    if (!edit) {
      ...
      const skuChanged = !isEqual(uniq(listSku), uniq(oldListSku));
      if (skuChanged) {
        setalreadyChanged(true);
      }
    }
  }, [watchAllFields, uniq(listSku).length, edit]);
  ...
  return (
    <>
            ...
            <PreviewCard className="" bodyClass="">
                  ...
                  <ul className="nk-block-tools g-3">
                    <li>
                      <p style={{ color: '#203864', fontSize: 12 }}>
                        INVENTORI&nbsp;&nbsp;/&nbsp;&nbsp;ADJUSTMENT&nbsp;&nbsp;/&nbsp;&nbsp;
                        <span style={{ color: '#BDC0C7' }}>
                          {edit ? 'Edit' : 'Tambah'} Adjustment
                        </span>
                      </p>
                    </li>
                  </ul>
              ...
              >
                <Row className="g-gs">
                  <BlockBetween>
                    <BlockHeadContent>
                      <h3 style={{fontSize: 32}}>{edit ? 'Edit' : 'Tambah'} Adjustment</h3>
                    </BlockHeadContent>
                  </BlockBetween>
                  <Col md="6">
                  ...
                  <Col md="6">
                    <div className="form-group">
                      ...
                      {edit ? (
                        <>
                          <TransferInfo.ContainerTime>
                          <Input value={getValues('adjustment_date')} disabled />
                          <Icon
                            className="position-absolute"
                            style={{
                              right: 0,
                                  ...
                                  <div className="form-control-wrap">
                                    <OrderInfo.ContainerTime>
                                      <DatePicker
                                        ...
                                        placeholderText={
                                          edit ? '' : 'Masukkan Waktu Adjustment'
                                        }
                                        onKeyDown={(e) => {
                                          e.preventDefault();
                                        }}
                              ...
                              <>
                                <div className="form-control-wrap">
                                  <DropdownOption
                                    ...
                                    options={getOptionStockInventoryAdjustment}
                                    className={
                  ...
                  <Col md="12">
                          ...
                          <Product.Table>
                            ...
                            >
                              <tr style={stylesFormInventory.header}>
                                ...
                                <th style={stylesFormInventory.listHeader}>Stok Adjustment<span style={{ color: 'red' }}>*</span></th>
                                <th style={stylesFormInventory.listHeader}>Perubahan</th>
                                {true && (
                            ...
                            {listSku.map((data, index) =>  (
                              <ListTableProduct
                                ...
                                adjustmentStock={data?.quantity || ''}
                                setAdjustmentStock={(value) =>
                                  onAdjustmentStock(
                                    index,
                                    'quantity',
                                    value
                                ...
                                currentStock={getValues('stock_source') === 'good' ? parseInt(data.goods_quantity || data.available_quantity) : parseInt(data.damages_quantity)}                                                                 
                                ...
                                <div className="d-flex justify-content-center">
                                  <Button
                                    ...
                                    className={`btn center shadow-none mt-2 ${
                                      !stockSourceValue || !originValue
                                      ? 'btn-disabled'
                                      : 'btn-primary'
                                    }`}
                                    disabled={!stockSourceValue || !originValue}
                                    onClick={() => {
                                      setshowAddSKUPage(true);
                                      !alreadyChangedSku &&
                                      setalreadyChangedSku(true);
                    ...
                    >
                      ...
                      <div className="text-danger">
                        Jumlah SKU dalam adjustment minimal 1
                      </div>
                    </div>
                  </Col>
                  <Col md="12">
                          ...
                          <li>
                            <Button
                              ...
                              className={`btn center shadow-none ${
                                ...
                                !isValid ||
                                listSku.length == 0 ||
                                listSku.some((v) => v.quantity === null || v.quantity === undefined || 
                                v.quantity === '' || isNaN(v.quantity))
                                  ? 'btn-disabled'
                                  : 'btn-primary'
                              }`}
                              type={!alreadyChanged || !isValid || listSku.length == 0 || 
                                listSku.some((v) => v.quantity === null || v.quantity === undefined || 
                                v.quantity === '' || isNaN(v.quantity)) 
                                 ? 'button' 
                                 : 'submit'
                              ...
                              disabled={
                                ...
                                !isValid ||
                                listSku.length == 0 ||
                                listSku.some((v) => v.quantity === null || 
      ...
      <AddSKUInventory
        ...
        werehouseOriginId={warehouseValue}
      />
      ...
      {ismodalSuccess && (
        <ModalConfirm
          ...
          title={`Berhasil\n${
            edit ? 'Memperbarui Adjustment!' : 'Membuat Daftar Adjustment!'
          }`}
          stylesCustomTitle={{
            paddingTop: 0,
          }}
