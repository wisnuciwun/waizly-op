...
import { TabOptionInventoryInbound } from '@/utils/constants';
import { getOptionInventoryInbound } from '@/utils/getSelectOption';
import { Input } from 'reactstrap';
import DatePicker from 'react-datepicker';
import { CSSProperties, useEffect, useState } from 'react';
import { useRouter } from 'next/router';
import TableInbound from '@/components/organism/table-inventory-inbound';
import { getDataListInbound } from '@/services/inventory';
...
function Inbound() {
  ...
  //   [],
  // );
  const [listDataInbound, setListDataInbound] = useState<InboundDataProps[]>(
    [],
  );
  ...
  const fetchDataList = async ({
    ...
    try {
      ...
      const payload = {
        ...
        size: size,
        search: {
          inbound_code: filterDataByType === 'order_code' ? search : '',
          sku_name: filterDataByType === 'sku_name' ? search : '',
          sku_code: filterDataByType === 'sku_code' ? search : '',
        },
      };
      const inBoundList = await getDataListInbound(payload);
      setListDataInbound(inBoundList?.data?.inbound);
      setPaginationConfig(() => ({
        current: page,
        record: size,
        totalData: inBoundList?.data?.page_info?.total_record,
      }));
    } catch (error) {
      console.log(error);
    } finally {
      ...
      <Content>
              ...
              <div>
                ...
                >
                  <Icon
                    ...
                    style={{ color: '#203864' }}
                  ></Icon>
                  <div style={{ paddingLeft: 3 }}>Unduh Inbound</div>
                </Button>
                ...
                >
                  <Icon
                    ...
                    style={{ color: '#203864' }}
                  ></Icon>
                  <div style={{ paddingLeft: 3 }}>Unggah Inbound</div>
                </Button> */}
              </div>
            </div>
          </BlockHeadContent>
        ...
        <BlockHeadContent>
          <div className="d-flex">
            <div className="mb-2">
              <TabsIcon
                tabsData={TabOptionInventoryInbound}
                activeTab={typeTabs}
                tabCounts={null}
                    ...
                    <div className="d-flex">
                      <div className="form-wrap">
                        <DropdownOption
                          ...
                          options={getOptionInventoryInbound}
                          optionLabel={'name'}
                          placeholder={'Pilih'}
